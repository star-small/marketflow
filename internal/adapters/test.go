// Temporarily replace internal/adapters/exchanges/test.go with this debug version
...
func NewTestExchangeAdapter(name string) port.ExchangeAdapter {
	slog.Info("Creating test exchange adapter", "name", name)
...
func (t *TestExchangeAdapter) Start(ctx context.Context) (<-chan domain.MarketData, error) {
	...
	t.isRunning = true
	slog.Info("Test adapter marked as running", "exchange", t.name)
...
func (t *TestExchangeAdapter) IsHealthy() bool {
	healthy := t.isRunning
	slog.Debug("Test adapter health check", "exchange", t.name, "healthy", healthy, "isRunning", t.isRunning)
	return healthy
}

func (t *TestExchangeAdapter) generateDataForSymbol(ctx context.Context, symbol string) {
	slog.Info("Starting data generation goroutine", "exchange", t.name, "symbol", symbol)

	defer func() {
		if r := recover(); r != nil {
			slog.Error("Panic in generateDataForSymbol", "exchange", t.name, "symbol", symbol, "panic", r)
		}
		slog.Info("Data generation goroutine ended", "exchange", t.name, "symbol", symbol)
	}()
	...
	slog.Info("Starting price generation", "exchange", t.name, "symbol", symbol, "basePrice", basePrice)

	// Random seed for this symbol to make it more realistic
	source := rand.NewSource(time.Now().UnixNano() + int64(len(symbol)*len(t.name)))
	rng := rand.New(source)

	// Generate price updates every 1-3 seconds for testing
	ticker := time.NewTicker(time.Duration(1000+rng.Intn(2000)) * time.Millisecond)
	defer ticker.Stop()
	...
	for {
		select {
		case <-ctx.Done():
			slog.Info("Context cancelled, stopping data generation", "exchange", t.name, "symbol", symbol, "dataCount", dataCount)
			return
		case <-t.stopChan:
			slog.Info("Stop signal received, stopping data generation", "exchange", t.name, "symbol", symbol, "dataCount", dataCount)
			return
		case <-ticker.C:
			if !t.isRunning {
				slog.Info("Adapter not running, stopping data generation", "exchange", t.name, "symbol", symbol)
				return
			}
			...
			dataCount++
			slog.Debug("Generated market data", "exchange", t.name, "symbol", symbol, "price", currentPrice, "count", dataCount)

			// Send to data channel if running
			if t.isRunning {
				select {
				case t.dataChan <- marketData:
					slog.Debug("Market data sent to channel", "exchange", t.name, "symbol", symbol, "price", currentPrice)
				case <-time.After(50 * time.Millisecond):
					slog.Warn("Data channel full, dropping market data", "exchange", t.name, "symbol", symbol)
				}
			}

			// Vary the ticker interval for more realistic data
			ticker.Reset(time.Duration(1000+rng.Intn(2000)) * time.Millisecond)
		}
	}
}
